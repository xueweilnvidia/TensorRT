    ...
    void loadXMMAKernels()
    {
        ...
        for (unsigned int i = 0; i < mKernelMetaCount; ++i)
        {
            ...
            if (kernelMeta.mSM == mSM && kernelMeta.mDataType == mDataType)
            {
                ...
                {
                    cuErrCheck(mDriver->cuModuleLoadData(&hmod, kernelMeta.mCubin), *mDriver.get());
                ...
                cuErrCheck(
                    mDriver->cuModuleGetFunction(&funcInfo.mDeviceFunction, hmod, kernelMeta.mFuncName), *mDriver.get());
...
public:
        ...
        unsigned int nbKernels, Data_type type, unsigned int sm)
    {
        ...
        if (findIter == mKernels.end())
        {
            ...
            std::cout<<"kernel size: " << mKernels.size()<<std::endl;
            return newKernel;
        }
...
public:
        ...
        bool paged_kv_input, bool has_qk_tanh_scale) const
    {
        ...
        uint64_t res_id = (uint64_t) s << 32 | d << 16 | (attention_mask_type << 9) | (has_qk_tanh_scale ? 256ull : 0ull)
            | (paged_kv_input ? 128ull : 0ull) | (is_alibi_supported ? 64ull : 0ull)
            | (warp_specialization ? 32ull : 0ull) | (tiled ? 16ull : 0ull) | (force_fp32_acc ? 8ull : 0ull)
            | (flash_attention ? 4ull : 0ull) | (interleaved ? 2ull : 0ull) | (unroll ? 1ull : 0ull);

        //  printf("FMHA kernels hash (kernel meta info: %d %d %d %d %d %d %d %d %d %d %d %d %ju) !",
        //     s, d, interleaved, unroll, force_fp32_acc,
        //     flash_attention, warp_specialization, is_alibi_supported,
        //     static_cast<int>(attention_mask_type), tiled,
        //     paged_kv_input, has_qk_tanh_scale, res_id);
    ...
    void run_template(Kernel_params& params, Launch_params& launch_params, cudaStream_t stream) const
    {
        ...
        if(findIter == mFunctions.end()){
            printf("FMHA kernels are not found (kernel meta info: %d %d %d %d %d %d %d %d %d %d %d %d) !",
            ...
            PLUGIN_VALIDATE(false);
        }
        // PLUGIN_VALIDATE_MSG(findIter != mFunctions.end(),
        ...
        { // forceunroll = true for flash attention kernels
            ...
            PLUGIN_VALIDATE_MSG(kernelMeta.mS == kernelMeta.mUnrollStep * unroll, "Wrong launching sequence length");
...
private:
        ...
        Launch_params const& launch_params) const
    {
        auto const findIter = mFunctions.find(hashFromParams(params, launch_params));
        if (findIter == mFunctions.end()){
            printf("FMHA kernels are not found (kernel meta info: %d %d %d %d %d %d %d %d %d %d) !", launch_params.kernel_s,
...
{
    std::cout<<"sm number: " <<sm<<std::endl;
