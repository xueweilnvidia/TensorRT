...
public:
    ...
    virtual void setup(int const b, int const s_q, int const s_kv, int const blocks_per_context_sequence,
        int const tokens_per_kv_block, int const sliding_window_size, int const total_seqlen, int const numHeads, int const headSize,
    ...
    virtual void setup(int const b, int const s, int const sliding_window_size, int const total_seqlen, int const numHeads, int const headSize) = 0;
    ...
    virtual void run(void const* qPtr, void const* pagedKVBlockOffsetsOnHost,
...
public:
    ...
    // Keep this for bert attention plugin.
    void setup(int const b, int const s, int const sliding_window_size, int const total_seqlen, int const numHeads, int const headSize)
    {
        setup(b, s, s, 0, 0, sliding_window_size, total_seqlen, numHeads, headSize);
    }

    bool fmha_supported() override;

    void run(void const* qPtr, void const* pagedKVBlockOffsetsOnHost,
        void const* cuQSeqlenPtr, void const* cuKVSeqlenPtr, uint32_t* tileCounterPtr, float const* scaleBmm2Ptr,
    ...
    void run(void const* inputPtr, void const* cuQSeqlenPtr, uint32_t* tileCounterPtr, float const* scaleBmm2Ptr,
        void* outputPtr, cudaStream_t stream)
    {
        run(inputPtr, nullptr,  cuQSeqlenPtr, cuQSeqlenPtr, tileCounterPtr, scaleBmm2Ptr, outputPtr,
            stream);
    }
