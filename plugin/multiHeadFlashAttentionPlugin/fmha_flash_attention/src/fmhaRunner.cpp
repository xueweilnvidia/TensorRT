...
static inline std::tuple<size_t, size_t> getDeviceMemoryInfo(bool const useUvm)
{
    if (useUvm)
    {
        ...
        printf("Using UVM based system memory for KV cache, total memory %0.2f GB, available memory %0.2f GB",
    ...
    {
        size_t free, total;
        PLUGIN_CHECK(cudaMemGetInfo(&free, &total));
...
public:
        ...
        , sm(sm_)
    {
        ...
        std::cout<<"before get xmma kernel"<<std::endl;
        xmmaKernel = getXMMAKernelsV2(mDataType, sm);
        std::cout<<"after get xmma kernel"<<std::endl;     
        mParams.clear();
        ...
        int const total_seqlen, bool const has_alibi, bool const scale_alibi, int const tp_size, int const tp_rank)
    {
        ...
        float const scale_after_alibi = scale_alibi ? inv_sqrt_scale : 1.0;
        float const scale_bmm1 = scale_alibi ? 1.0 : inv_sqrt_scale;
        float const scale_softmax = 1.0; // Seems to be only required for int8
        float const scale_bmm2 = 1.0;
        ...
        // printf("inv_sqrt_scale: %f scale_bmm1: %f scale_softmax: %f scale_bmm2: %f mLaunchParams.useBase2ExpTrick %d \n params.b:  %d params.h: %d params.s: %d params.d: %d params.sliding_window_size: %d\n params.o_stride_in_bytes: %d mTotalSeqLen %d",
        //     inv_sqrt_scale, scale_bmm1, scale_softmax, scale_bmm2, static_cast<int>(mLaunchParams.useBase2ExpTrick),
        //     params.b, params.h, params.s, params.d, params.sliding_window_size, params.o_stride_in_bytes, mTotalSeqLen);
    }

    // Support packed QKV.
    void setup(int const b, int const s, int const sliding_window_size, int const total_seqlen, bool const has_alibi,
        bool const scale_alibi, int const tp_size, int const tp_rank, int const numHeads, int const headSize)
    {
        ...
        PLUGIN_VALIDATE_MSG(mHeadSize > 0, "Head size should be greater than 0.");
        ...
        // printf("mParams.h_kv: %d ", mParams.h_kv);
...
FusedMHARunnerV2::FusedMHARunnerV2(const Data_type data_type, bool const pagedKVFMHA, int const numHeads,
    ...
    : pimpl(new mhaImpl(
        data_type, pagedKVFMHA, numHeads, headSize, qScaling, qkTanhScale, bert::getSMVersion()))
{
}
...
void FusedMHARunnerV2::setup(int const b, int const s_q, int const s_kv, int const blocks_per_context_sequence,
    int const tokens_per_kv_block, int const sliding_window_size, int const total_seqlen, int const numHeads,
    int const headSize, bool const has_alibi,
    bool const scale_alibi, int const tp_size, int const tp_rank)
{
    ...
    {
        pimpl->setup(b, s_q, sliding_window_size, total_seqlen, has_alibi, scale_alibi, tp_size, tp_rank, numHeads, headSize);
    }
}
...
void FusedMHARunnerV2::run(void const* qPtr, void const* pagedKVBlockOffsetsOnHost,
    void const* cuQSeqlenPtr, void const* cuKVSeqlenPtr, uint32_t* tileCounterPtr, float const* scaleBmm2Ptr,
    void* outputPtr, cudaStream_t stream)
{
    if (pimpl->use_paged_kv_fmha())
...
{
    std::cout<<"isValid start" <<std::endl;
